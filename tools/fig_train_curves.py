#!/usr/bin/env python
"""tools/fig_train_curves.py -- training curves from a run's metrics JSONL
(written by infra RunLogger): per-term losses, effective rank (the collapse
gate's history), throughput, timer splits. Overlay up to 4 runs.

Usage: tools/fig_train_curves.py artifacts/experiments/jepa_pretrain_metrics.jsonl \
