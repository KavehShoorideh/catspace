#!/usr/bin/env python
"""experiments/viz_b_mate_clusters.py -- Kaveh 2026-07-22: "I want to see clusters in B
...
def harvest_approaches(tb, rng, n_games, tail=8):
    ...
    import json
    fens = json.loads(Path("artifacts/experiments/krrkbp_test_n200.json").read_text())["fens"]
...
def main():
    ...
    ap.add_argument("--fields", nargs="+", default=["data/derived/sep/iqe_geom_field.pt",
                                                    "data/derived/sep/nucleus_distilled.pt"])
    ...
    ap.add_argument("--out", default="artifacts/experiments/b_mate_clusters.png")
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()
    t0 = time.time(); dev = pick_device(args.device); rng = np.random.default_rng(args.seed)

    mates, labels = harvest_mates("data/derived/dtm_endgame.npz", args.n_mates, rng)
    tb = TB("data/syzygy")
