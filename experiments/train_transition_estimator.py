#!/usr/bin/env python
"""experiments/train_transition_estimator.py -- M2a: the TRANSITION ESTIMATOR T. An MLP head over the
...
def run_variant(phi, ctx, y, games, dev, steps, batch, val_mask, seed, name):
    ...
    cfg = TrainConfig(out=f"artifacts/experiments/T_{name}", steps=steps, ckpt_every=steps,
