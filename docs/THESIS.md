# THESIS — the hypothesis, the formal frame, the architecture

The document of record for what catspace is trying to prove and how the system
is built. Short version: **humans play strong chess on 10–100 positions of
search because they plan over subgoals with a model of their adversary; an
engine built to plan the same way should yield interpretability as a
consequence, not a feature.** Strength-per-node is the frontier; interpretability
(faithfulness, legibility, calibration, concept alignment) is the measured
primary endpoint. Engine-competitive strength is explicitly not the goal.
...
gradients. Full spec: [SUBGOALFORMER.md](SUBGOALFORMER.md). Earlier
