...
from catspace.chain import TransitionChain
from catspace.cone.embedding import QuasimetricEmbedding
from catspace.scoring import TerminalScores, dtm_filled, fill_terminal_state_scores
from catspace.planner.readout import ReplyAgg, backup, greedy_policy, move_values
...
class PlanningPolicy:
    ...
    def _values(self, goal) -> np.ndarray:
        if goal.name not in self._V:
            from catspace.cone.embedding import reach as reach_fn
    ...
    def move_id(self, chain: TransitionChain, s: int, rng: np.random.Generator) -> int:
        from catspace.planner.selector import GreedyReach
