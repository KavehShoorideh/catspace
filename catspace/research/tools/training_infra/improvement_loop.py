#!/usr/bin/env python
"""catspace/research/tools/training_infra/improvement_loop.py -- THE UNIFIED IMPROVEMENT LOOP (Kaveh 2026-07-25:
...
EPTR = Path(paths.sep("opponent_energy_current.txt"))
MAIA = [paths.engine("maia/maia-1100.pb.gz"), paths.engine("maia/maia-1200.pb.gz"),
        paths.engine("maia/maia-1400.pb.gz"), paths.engine("maia/maia-1600.pb.gz"),
        paths.engine("maia/maia-1900.pb.gz")]   # full rung rotation (camping phase 2)


def field_now():
    return PTR.read_text().strip() if PTR.exists() else paths.sep("lichess_mc2.pt")
...
def main():
    ...
    ap.add_argument("--base-shards",
                    default=paths.shards("lichess_db_standard_rated_2019-01.prefix4gb"))
    ap.add_argument("--self-shards", default=paths.shards("self_play_v1"))
    args = ap.parse_args()
    t0 = time.time()
    store = ExperienceStore()
    bpfx = paths.experiment("improve_persistent")
    ...
    for rnd in range(args.rounds):
        ...
        while True:
            rc = subprocess.run([sys.executable, "catspace/approaches/bootstrap_mate/experiments/bootstrap_mate_engine.py",
                                 ...
                                 "--results-file",
                                 paths.experiment(f"improve_r{rnd}_results.jsonl")]).returncode
            if rc != 75:
                break
        ...
        fck = paths.sep(f"self_field_r{k}.pt")
        ...
        rc = subprocess.run([sys.executable, "catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/train_lichess_fb.py",
                           ...
                           stdout=open(paths.experiment(f"improve_field_r{rnd}.log"), "w"),
                           stderr=subprocess.STDOUT).returncode
        if rc == 0:
            PTR.write_text(fck)
            print(f"[improve] FIELD -> {fck}", flush=True)
        # b. planner RL refit on everything
        pk = len(list(Path(str(paths.sep_dir())).glob("planner_rl_r*.pt")))
        rc = subprocess.run([sys.executable, "catspace/research/components/planner/approaches/subgoal_cascade/experiments/train_planner_rl.py",
                            "--out", paths.sep(f"planner_rl_r{pk}.pt")],
                           stdout=open(paths.experiment(f"improve_rl_r{rnd}.log"), "w"),
                           stderr=subprocess.STDOUT).returncode
        for line in open(paths.experiment(f"improve_rl_r{rnd}.log")):
            if "VERDICT" in line:
        ...
        ms_npz = paths.derived("move_selection_self.npz")
        subprocess.run([sys.executable, "catspace/research/components/planner/approaches/subgoal_cascade/experiments/build_move_selection.py",
                        "--shards", args.self_shards, "--n-rows", "200000",
                        "--out", ms_npz],
                       stdout=open(paths.experiment(f"improve_energy_r{rnd}.log"), "w"),
        ...
        if n_self < 200:
            ...
            energy = EPTR.read_text().strip() if EPTR.exists() \
                else paths.sep("opponent_energy_v1.pt")
            ek = len(list(Path(str(paths.sep_dir())).glob("opponent_energy_r*.pt")))
            eck = paths.sep(f"opponent_energy_r{ek}.pt")
            rc = subprocess.run([sys.executable, "catspace/research/components/planner/approaches/opponent_model/experiments/train_opponent_model.py",
                                "--data",
                                paths.derived(f"move_selection_full_v1.npz:0.65,{ms_npz}:0.35"),
                                "--init", energy, "--steps", "2000",
                                "--out", eck],
                               stdout=open(paths.experiment(f"improve_energy_r{rnd}.log"), "a"),
                               stderr=subprocess.STDOUT).returncode
            if rc == 0:
                EPTR.write_text(eck)
                print(f"[improve] ENERGY -> {eck}", flush=True)
            for line in open(paths.experiment(f"improve_energy_r{rnd}.log")):
                if "VERDICT" in line:
                    print("  " + line.strip(), flush=True)
