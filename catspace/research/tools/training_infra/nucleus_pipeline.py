#!/usr/bin/env python
"""catspace/research/tools/training_infra/nucleus_pipeline.py -- PROGRESSIVE GENERAL-NUCLEUS PIPELINE (Kaveh
...
def main():
    ...
    ap.add_argument("--out-dir", default=paths.derived("nucleus"))
    args = ap.parse_args()
    t0 = time.time()
    ...
    for k in range(args.rounds):
        ...
        if not shard.exists():
            if gen_proc is None:                 # round 0 gen, or resume
                gen_proc = run([sys.executable, "catspace/research/components/planner/approaches/endgame_groundtruth/experiments/gen_dtm_data.py",
                                "--materials", "all", "--per", str(delta),
                                "--seed", str(1000 * k), "--workers", str(args.workers),
                                "--out", str(shard)],
                               paths.experiment(f"nucleus_gen_r{k}.log"))
        ...
        if k + 1 < args.rounds:
            ...
            if not nshard.exists():
                ...
                gen_proc = run([sys.executable, "catspace/research/components/planner/approaches/endgame_groundtruth/experiments/gen_dtm_data.py",
                                ...
                                "--out", str(nshard)],
                               paths.experiment(f"nucleus_gen_r{k+1}.log"))
        ...
        ckpt = paths.sep(f"dtm_tok_r{k}.pt")
        print(f"[pipeline] training round {k} on {k+1} shard(s) [{time.time()-t0:.0f}s]",
              flush=True)
        rc = subprocess.run([sys.executable, "catspace/research/components/encoder/approaches/jepa_tokenizer/experiments/train_dtm_tok.py",
                             "--dtm-npz", shards, "--steps", str(4000 + 2000 * k),
                             "--out", ckpt],
                            stdout=open(paths.experiment(f"nucleus_train_r{k}.log"), "w"),
                            stderr=subprocess.STDOUT).returncode
        if rc != 0:
            print(f"[pipeline] TRAIN round {k} FAILED rc={rc}; continuing gen, skipping flip",
                  flush=True)
            continue
        for line in open(paths.experiment(f"nucleus_train_r{k}.log")):
            if "VERDICT" in line:
