#!/usr/bin/env python
"""catspace/research/tools/training_infra/expert_iteration.py -- the mate mission's legitimate engine (DECISIONS 4b):
...
from catspace.research.components.planner.approaches.endgame_groundtruth.experiments.ladder_mate import make_dtm_value, white_mcts
...
def main():
    ...
    ap.add_argument("--base-ckpt", default=paths.sep("dtm_cnn_v2.pt"))
    ap.add_argument("--base-data", default=paths.derived("dtm_endgame_v2.npz"))
    ...
    ap.add_argument("--harvest-out", default=paths.derived("ei_harvest.npz"))
    ...
    for r in range(1, args.rounds + 1):
        ...
        new_ckpt = paths.sep(f"dtm_cnn_ei_r{r}.pt")
        rc = subprocess.run([PY, "catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_dtm_cnn.py", "--dtm-npz", merged,
        ...
        ckpt = new_ckpt
        # the exam
        rc = subprocess.run([PY, "catspace/research/components/planner/approaches/endgame_groundtruth/experiments/mate_ladder_eval.py", "--configs", "dtm",
