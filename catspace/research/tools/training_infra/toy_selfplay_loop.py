#!/usr/bin/env python
"""
catspace/research/tools/training_infra/toy_selfplay_loop.py — the closed loop on ONE toy scenario.
...
def main():
    ...
    ap.add_argument("--init-ckpt", default=paths.derived("lichess_fb_4gb_qm_plygap_only.pt"))
    ap.add_argument("--human-shards", default=paths.shards("lichess_db_standard_rated_2019-01.prefix4gb"))
    ap.add_argument("--start-fens", default=paths.experiment("krrkbp_fixed_set_n60.json"))
    ...
    ap.add_argument("--replay-dir", default=paths.selfplay("krrkbp_loop"))
    ap.add_argument("--work-dir", default=paths.derived("krrkbp_loop"))
    ap.add_argument("--log", default=paths.generated("toy_selfplay_loop.log"))
    ...
    with logp.open("a") as log:
        ...
        for r in range(1, args.rounds + 1):
            ...
            tmp = ROOT / paths.selfplay(f"_krrkbp_r{r}_tmp")
            if tmp.exists():
                shutil.rmtree(tmp)
            sh([PY, "catspace/research/components/memory/approaches/experience_store/experiments/selfplay_generate.py", "--ckpt", str(current),
            ...
            sh([PY, "catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/train_lichess_fb.py", "--ckpt", str(round_ckpt),
            ...
            sh([PY, "catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/reach_curvature.py", "--ckpt", str(current),
