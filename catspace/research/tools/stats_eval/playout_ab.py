#!/usr/bin/env python
"""
catspace/research/tools/stats_eval/playout_ab.py — paired PLAYOUT A/B with a DETERMINISTIC defender.
...
def mate_vector(ckpt, starts, tb, nodes, beam, max_plies, seed, device, bank_boards=None,
    ...
    if rescue:
        ...
        for t in ("certainty_table_demo_tb", "certainty_table_eps05", "certainty_table_r2_K16"):
            path = Path(paths.experiment(f"{t}.json"))
            if not path.exists():
                continue
...
def main():
    ...
    ap.add_argument("--bank-shards", nargs="+", default=[paths.selfplay("krrkbp_sfsf")])
    ap.add_argument("--bank-max-pieces", type=int, default=6)
    ap.add_argument("--bank-size", type=int, default=128)
    ap.add_argument("--search-a", choices=("beam", "mcts", "anytime", "mctsplan"), default="beam")
    ap.add_argument("--search-b", choices=("beam", "mcts", "anytime", "mctsplan"), default="beam",
