#!/usr/bin/env python
"""
catspace/research/tools/stats_eval/repro_check.py — re-runs the ported pipeline and diffs against
...
(catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py) and band-checks the curriculum/search-sweep
...
def check_krk_rung1():
    ...
    out = run_capture(["catspace/research/components/planner/approaches/endgame_groundtruth/experiments/krk_rung1.py"])
...
def check_krkn_search_sweep():
    ...
    if not (ROOT / "data" / "derived" / "krkn_F.npy").exists():
        print("SKIP: data/derived/krkn_F.npy missing -- run catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py first")
        return
    out = run_capture(["catspace/research/components/planner/approaches/endgame_groundtruth/experiments/krkn_search_sweep.py"])
    k0 = grab(r"0 \| +1 +\| +([\d.]+)", out)
...
def check_full_curriculum():
    ...
    out = run_capture(["catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py"])
