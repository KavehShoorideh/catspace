#!/usr/bin/env python
"""
catspace/research/tools/stats_eval/compare_methods.py — paired, e-value-tested comparisons between
...
Requires catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py (or train_krk_pi.py for --domain krk) to
    ...
    if F is None:
        if domain_name == "krk":
            ...
            raise FileNotFoundError(
                f"{paths.derived(f'{domain_name}_F.npy')} not found -- "
                f"run the {domain_name} trainer first")
    else:
