#!/usr/bin/env python
"""catspace/research/tools/stats_eval/integration_tests.py -- THE REGRESSION GATE (Kaveh: 'future works have to
...
def main():
    ...
    fen_file = Path(paths.experiment(f"{args.tag}_fens.txt"))
    fen_file.write_text("\n".join(c["fen"] for c in cases))
    res_file = Path(paths.experiment(f"{args.tag}_results.jsonl"))
    res_file.unlink(missing_ok=True)
    for sfx in ("_bank", "_lossbank", "_drawbank"):
        Path(paths.experiment(f"{args.tag}{sfx}.fens")).unlink(missing_ok=True)

    if args.import_banks == "MERGE":
        import glob
        pfx = paths.experiment(f"{args.tag}_seed")
        for sfx, pat in (("_bank", "wdlr6_bank_n5000_*.fens"),
                         ("_lossbank", "wdlr6_lossbank_n5000_*.fens"),
                         ("_drawbank", "wdlr6_drawbank_n5000_*.fens")):
            lines = set()
            for f in glob.glob(paths.experiment(f"{pat}")):
    ...
    cmd = [sys.executable, "catspace/approaches/bootstrap_mate/experiments/bootstrap_mate_engine.py",
           ...
           "--bank-file", paths.experiment(f"{args.tag}_bank.fens"),
           "--loss-bank-file", paths.experiment(f"{args.tag}_lossbank.fens"),
           "--draw-bank-file", paths.experiment(f"{args.tag}_drawbank.fens"),
           "--milestone-file", paths.experiment(f"{args.tag}_ms.jsonl"),
