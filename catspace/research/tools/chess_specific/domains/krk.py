...
from catspace.research.tools.chess_specific.board import N, NSQ, KING_MOVES, rc, rook_moves, rook_attacks, chebyshev, sq
from catspace.research.tools.chess_specific.chain import build_csr, Terminals, TransitionChain, KIND_ONGOING, KIND_MATE, KIND_STALEMATE
...
def black_moves(wk, wr, bk):
    ...
    for t in KING_MOVES[bk]:
        ...
        if t == wr:
            ...
            if rook_attacks(wr, t, {wk}): continue  # can't move into check
...
def box_area(wk, wr, bk):
    ...
    walls = {wr}
    for t in range(NSQ):
        if t != wk and rook_attacks(wr, t, {wk}):
            walls.add(t)
