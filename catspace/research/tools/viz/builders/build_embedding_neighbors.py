#!/usr/bin/env python
"""
catspace/research/tools/viz/builders/build_embedding_neighbors.py — "does the embedding make sense?"
...
def main():
    ...
    ap.add_argument("--ckpt", default=paths.derived("lichess_fb_4gb_qm_plygap_only.pt"))
    ap.add_argument("--bank-shards", nargs="+", default=[paths.selfplay("krrkbp_loop")])
    ap.add_argument("--fixed-set", default=paths.experiment("krrkbp_fixed_set_n60.json"))
    ...
    out = Path(args.out) if args.out else Path(str(paths.generated_dir())) / f"embed_neighbors_{label}.html"
