#!/usr/bin/env python
"""
catspace/research/tools/viz/builders/play_server.py — COMPONENT B of the play-atlas app.
...
exists, then:
  ...
  .venv/bin/python catspace/research/tools/viz/builders/play_server.py --port 8000
...
ATLAS_DIR = ROOT / paths.generated("play_atlas")
...
class Engine:
    ...
    def rebuild_atlas(self, n=6000, algo="tsne", params=None):
        ...
        cmd = [_sys.executable, str(ROOT / "catspace/research/tools/viz/builders/build_play_atlas.py"),
...
def veto_analyze(fen: str | None, h: int = 4, n_targets: int = 16) -> dict:
    ...
    try:
        with open(paths.experiment("veto_lab_trail.jsonl"), "a") as fh:
...
def main():
    ...
    ap.add_argument("--memory", default=paths.derived("position_memory"),
                    help="position-memory dir (vector DB of seen positions + outcomes; "
                         "build with catspace/research/components/planner/approaches/subgoal_cascade/experiments/build_position_memory.py). Serves "
    ...
    ap.add_argument("--root-min-visits", type=int, default=10,
                    help="CI-driven root exploration: every non-terminal root move gets "
                         ...
                         "plain PUCT root.")
    ap.add_argument("--planner-field", default=paths.sep("nucleus_distilled.pt"),
    ...
    if args.planner:
        from catspace.research.components.planner.approaches.endgame_groundtruth.experiments.longshort_engine import LongShortEngine
        print(f"long/short PLANNER engine ON: field {Path(args.planner_field).name}", flush=True)
        _dz = np.load(paths.derived("dtm_endgame.npz"))
    ...
    if not (ATLAS_DIR / "atlas.json").exists():
        print("WARNING: atlas.json missing — run catspace/research/tools/viz/builders/build_play_atlas.py "
