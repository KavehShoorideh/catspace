#!/usr/bin/env python
"""catspace/research/tools/viz/m3_atlas_viz.py -- the M3 atlas artifact: the subgoal map a planner navigates.
...
def main():
    t = np.load(paths.reach("region_table_v2.npz"), allow_pickle=True)
    ...
    fig.tight_layout()
    out = Path(paths.experiment("m3_atlas.png"))
    fig.savefig(out, dpi=130)
    Path(str(paths.figures_dir())).mkdir(exist_ok=True, parents=True)
    fig.savefig(paths.figure("m3_atlas.png"), dpi=130)
