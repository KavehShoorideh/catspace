"""catspace.research.catspace/research/infra/observability/run_metrics.py -- one metrics stream per training run,
...
context managers; their totals ride along on the next log() row as <name>_s.
    ...
    log = RunLogger(paths.experiment("jepa_pretrain"))
    with log.timer("data"):   ...
    with log.timer("fwd"):    ...
    log.log(step=1000, l_dyn=0.12, eff_rank=41.2)

Consumers: catspace/research/tools/figures/fig_train_curves.py (loss/rank/throughput panels),
catspace/research/tools/figures/fig_probe_curve.py --loss (step:loss pairs).
"""
