#!/usr/bin/env python
"""catspace/research/components/encoder/approaches/reachability_field/experiments/validate_decoupled.py -- acceptance test for the DECOUPLED field design
...
from catspace.research.tools.chess_specific.chessdata.encode import board_from_packed, encode_meta, encode_packed  # noqa: F401
...
def main():
    ...
    Path(str(paths.sep_dir())).mkdir(parents=True, exist_ok=True)
    torch.save({"state": head.state_dict(), "enc_out": enc_out, "scale": args.scale,
                "field": str(args.field)}, paths.sep("dtm_head_trunk.pt"))
