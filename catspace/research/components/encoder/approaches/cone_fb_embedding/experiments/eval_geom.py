#!/usr/bin/env python
"""catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/eval_geom.py -- held-out evaluation of the geometry field (board-only).
...
Usage: .venv/bin/python catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/eval_geom.py data/derived/sep/iqe_geom.pt
"""
...
ckpt = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(paths.sep("iqe_geom.pt"))
...
nz = np.load(paths.derived("lichess_nearmate.npz")); won = np.flatnonzero(nz["dtm"] > 0)
