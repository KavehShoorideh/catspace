#!/usr/bin/env python
"""catspace/research/components/encoder/approaches/control_field_wdl/experiments/audit_channel_balance.py -- the pre-launch data audit that would have caught
...
def main():
    ...
    sources = [
        ("regime0 human (4gb)", paths.shards("lichess_db_standard_rated_2019-01.prefix4gb")),
        ("regime1 random-mid", paths.shards("regime_random_v1")),
        ("regime1b random-END", paths.shards("regime_random_endgame_v1")),
        ("regime2 sf_cont", paths.shards("sf_cont_endgame_v1")),
    ]
    ...
    ms = Path(paths.derived("move_selection_v1.npz"))
    if ms.exists():
        z = np.load(ms)
        co = z["cohort"]
