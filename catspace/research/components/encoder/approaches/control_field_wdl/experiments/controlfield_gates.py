#!/usr/bin/env python
"""catspace/research/components/encoder/approaches/control_field_wdl/experiments/controlfield_gates.py -- Phase 2.3 validation gates for the control
...
def gate1_sanity(n, engine, depth, seed=0):
    ...
    t = pq.read_table(paths.records("lichess_2019-01/records_00000.parquet")).to_pylist()
    rng.shuffle(t)
    cone_sizes, evals = [], []
    tried = 0
    for g in t:
...
def gate2_puzzles(n, theme_filter, seed=0):
    ...
    with open(str(paths.puzzle_db()), "rb") as f:
