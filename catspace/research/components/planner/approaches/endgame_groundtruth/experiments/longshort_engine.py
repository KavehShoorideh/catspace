#!/usr/bin/env python
"""catspace/research/components/planner/approaches/endgame_groundtruth/experiments/longshort_engine.py -- the long/short planner engine (Kaveh 2026-07-21 UI spec): the FIELD does
...
def _smoke():
    ...
    ap.add_argument("--field", default=paths.sep("nucleus_distilled.pt"))
    ap.add_argument("--n", type=int, default=6); ap.add_argument("--nodes", type=int, default=400)
    args = ap.parse_args()
    dev = pick_device("cpu"); rng = np.random.default_rng(0); tb = TB(str(paths.syzygy_dir()))
    dz = np.load(paths.derived("dtm_endgame.npz")); idx = rng.permutation(len(dz["packed"]))[:1500]
    eng = LongShortEngine(args.field, dz["packed"][idx], dz["meta"][idx], dev, nodes=args.nodes)
    import json
    starts = [chess.Board(f) for f in json.loads(Path(paths.experiment("krrkbp_test_n200.json")).read_text())["fens"][:args.n]]
    mated = 0
    for s in starts:
