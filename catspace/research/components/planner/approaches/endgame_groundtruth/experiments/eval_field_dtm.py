#!/usr/bin/env python
"""catspace/research/components/planner/approaches/endgame_groundtruth/experiments/eval_field_dtm.py -- the DTM-ORDER measurement pass (Kaveh 2026-07-20).
...
Usage:
  .venv/bin/python catspace/research/components/planner/approaches/endgame_groundtruth/experiments/eval_field_dtm.py data/derived/sep/iqe_infinite_gn.pt
"""
...
dev = "cpu"
ckpt = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(paths.sep("iqe_infinite_gn.pt"))
fb, pay = load_ckpt(ckpt, dev); fb.eval()
om = omega_ids(np.array([1800]), np.array([1800]), np.array([300.0]))[0]
nz = np.load(paths.derived("lichess_nearmate.npz")); won = nz["dtm"] > 0
...
pz = np.load(paths.derived("pawndeath_pairs.npz"))
