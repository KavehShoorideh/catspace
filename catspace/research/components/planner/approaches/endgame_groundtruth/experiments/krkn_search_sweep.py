#!/usr/bin/env python
"""
catspace/research/components/planner/approaches/endgame_groundtruth/experiments/krkn_search_sweep.py — port of code/exp_search.py onto the shared
...
Requires catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py to have been run first (reads
...
from catspace.research.tools.chess_specific.arena import evaluate as arena_evaluate
from catspace.research.tools.chess_specific.domains import krkn
from catspace.io.paths import generated_dir, load_array
from catspace.research.tools.chess_specific.opponents import TableOpponent, optimal_reply_table
from catspace.research.components.planner.approaches.subgoal_cascade.src.policy import TablePolicy
from catspace.research.components.planner.approaches.subgoal_cascade.src.readout import ReplyAgg, backup, greedy_policy
...
def main():
    ...
    try:
        ...
        print(f"{paths.derived('{dtm_krkn,krkn_F,krkn_B}.npy')} not found -- "
              "run train_krkn.py in this approach's experiments/ first.", file=sys.stderr)
        return 1
    ...
    try:
        ...
        from catspace.research.tools.viz.viz.projection import PCAProjection
