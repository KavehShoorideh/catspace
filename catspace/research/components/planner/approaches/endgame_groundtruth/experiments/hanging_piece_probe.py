#!/usr/bin/env python
"""catspace/research/components/planner/approaches/endgame_groundtruth/experiments/hanging_piece_probe.py -- v2 of the defender-circuit probe
(supersedes catspace/research/components/planner/approaches/endgame_groundtruth/experiments/defender_circuit_probe.py's case/control definition; see
...
Stockfish-computed (depth 12, WDL win-fraction) by catspace/research/components/planner/approaches/endgame_groundtruth/experiments/sf_label_transitions.py
...
Usage:
  catspace/research/components/planner/approaches/endgame_groundtruth/experiments/hanging_piece_probe.py --hang-thr 0.25 --fair-thr 0.05
"""
...
from catspace.research.components.planner.approaches.endgame_groundtruth.experiments.defender_circuit_probe import encode_squares, encode_squares_from_embed  # noqa: E402
...
def main():
    ...
    ap.add_argument("--out", default=paths.experiment("hanging_piece_probe"))
    args = ap.parse_args()
    t0 = time.time()
    ...
    # Hewitt&Liang control task (catspace/research/components/encoder/approaches/reachability_field/experiments/probe_readout.py), game-level held-out split ----
    from catspace.research.components.encoder.approaches.reachability_field.experiments.probe_readout import linear_readout, mlp_readout_with_control
    gs = np.unique(groups)
    ...
    fig.savefig(paths.figure(f"{Path(args.out).name}.png"), dpi=130)
