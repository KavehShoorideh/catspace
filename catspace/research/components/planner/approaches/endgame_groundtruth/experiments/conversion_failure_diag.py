...
dev = "cpu"
fb, pay = load_ckpt(Path(paths.sep("cert_base_full.pt")), dev); fb.eval()
hp = torch.load(paths.sep("cert_base_full_phead.pt"), map_location=dev, weights_only=False)
...
starts = json.loads(Path(paths.experiment("krrkbp_test_n200.json")).read_text())["fens"][:N]
