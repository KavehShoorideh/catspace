...
from catspace.research.tools.chess_specific.chain import TransitionChain
from catspace.research.components.encoder.approaches.cone_fb_embedding.src.embedding import QuasimetricEmbedding
from catspace.research.tools.chess_specific.scoring import TerminalScores, dtm_filled, fill_terminal_state_scores
from catspace.research.components.planner.approaches.subgoal_cascade.src.readout import ReplyAgg, backup, greedy_policy, move_values
...
class PlanningPolicy:
    ...
    def _values(self, goal) -> np.ndarray:
        if goal.name not in self._V:
            from catspace.research.components.encoder.approaches.cone_fb_embedding.src.embedding import reach as reach_fn
    ...
    def move_id(self, chain: TransitionChain, s: int, rng: np.random.Generator) -> int:
        from catspace.research.components.planner.approaches.subgoal_cascade.src.selector import GreedyReach
