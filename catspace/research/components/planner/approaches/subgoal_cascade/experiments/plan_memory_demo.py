#!/usr/bin/env python
"""
catspace/research/components/planner/approaches/subgoal_cascade/experiments/plan_memory_demo.py — plan memory on KRkn: two goals (direct MATE,
...
Requires catspace/research/components/planner/approaches/endgame_groundtruth/experiments/train_krkn.py to have been run at least once (reads
...
def main():
    ...
    try:
        ...
        print(f"{paths.derived('{dtm_krkn,krkn_F,krkn_B}.npy')} not found -- "
              "run train_krkn.py under planner/endgame_groundtruth first.", file=sys.stderr)
        return 1
