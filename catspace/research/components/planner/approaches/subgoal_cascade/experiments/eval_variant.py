#!/usr/bin/env python
"""
catspace/research/components/planner/approaches/subgoal_cascade/experiments/eval_variant.py — one-command evaluation of an embedding variant for
...
Prints one VERDICT line. Run: python catspace/research/components/planner/approaches/subgoal_cascade/experiments/eval_variant.py --ckpt <c> --label <name>
"""
...
PY = sys.executable
INCUMBENT = paths.derived("lichess_fb_4gb_qm_plygap_only.pt")
...
def main():
    ...
    ap.add_argument("--out", default=paths.experiment("overnight_results.jsonl"))
    args = ap.parse_args()

    rec = {"label": args.label, "ckpt": args.ckpt, "note": args.note}

    # curvature (fast)
    cout = run([PY, "catspace/research/components/encoder/approaches/cone_fb_embedding/experiments/reach_curvature.py", "--ckpt", args.ckpt,
    ...
    vout = run([PY, "catspace/research/components/planner/approaches/endgame_groundtruth/experiments/conversion_compare.py", "--ckpt-a", INCUMBENT,
