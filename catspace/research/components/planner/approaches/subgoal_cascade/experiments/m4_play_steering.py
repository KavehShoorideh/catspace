#!/usr/bin/env python
"""catspace/research/components/planner/approaches/subgoal_cascade/experiments/m4_play_steering.py -- M4 first play harness: planner-ON vs planner-OFF vs Maia.
...
from catspace.approaches.gauntlet_harness.experiments.play_vs_maia import CommittorGreedy                    # noqa: E402
...
def main():
    ...
    ap.add_argument("--field", default=paths.experiment("reach_v2_full_latest.pt"))
    ap.add_argument("--reach", default=paths.reach("reach_v2.npz"))
    ap.add_argument("--table", default=paths.reach("region_table_v2.npz"))
    ap.add_argument("--store", default=paths.derived("engine_memory.sqlite"))
    ...
    ap.add_argument("--save-pgn", default=paths.experiment("m4_steering.pgn"))
    args = ap.parse_args()
    ...
    for arm in ("off", "on"):
        ...
        for g in range(args.games):
            ...
            pol.live = LiveOpponent(rf, m2, m2_inf, prepared,
                                    ...
                                    paths.derived("m2b/cache_3k.npz"),
