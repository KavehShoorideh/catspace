#!/usr/bin/env python
"""
catspace/research/components/planner/approaches/committor_value/experiments/committor_root_loop.py — the closed loop from THE root (Kaveh GO,
...
from catspace.research.components.memory.approaches.experience_store.experiments.selfplay_generate import KRRKBP_FIXED_START
...
PY = sys.executable
EXP = Path(str(paths.experiments_dir()))
...
def paired_rho(ckpt_a, whead_a, ckpt_b, whead_b, table_path, rim_plies=8.0, seed=0,
    ...
    from catspace.research.components.encoder.approaches.reachability_field.experiments.certainty_distill import spearman_ci
...
def main():
    ...
    ap.add_argument("--ckpt-in", default=paths.sep("committor_joint.pt"))
    ...
    for r in range(args.start_round, args.start_round + args.rounds):
        ...
        for w in range(args.workers):
            ...
            cmd = [PY, "-u", "catspace/research/components/encoder/approaches/reachability_field/experiments/certainty_rollouts.py",
        ...
        out = run([PY, "-u", "catspace/research/components/memory/approaches/experience_store/experiments/table_from_dump.py", "--dump", *dumps,
        ...
        cand = paths.sep(f"{args.tag}_r{r}.pt")
        out = run([PY, "-u", "catspace/research/components/planner/approaches/committor_value/experiments/committor_distill.py", "--loss", "mse",
