#!/usr/bin/env python
"""catspace/research/components/planner/approaches/opponent_model/experiments/build_home_book.py -- the HOME BOOK (Kaveh 2026-07-30): per-cell measurement of
...
def main():
    ...
    ap.add_argument("--pgns", nargs="+", default=sorted(
        glob.glob(paths.experiment("m4_steering_*.pgn"))
        + glob.glob(paths.experiment("field_v3_*maia*.pgn"))
    ...
    ap.add_argument("--field", default=paths.experiment("reach_v3_full_latest.pt"))
    ap.add_argument("--reach", default=paths.reach("reach_v3.npz"))
    ap.add_argument("--table", default=paths.reach("region_table_v3.npz"))
    ...
    ap.add_argument("--out", default=paths.reach("home_book_v1.npz"))
    ap.add_argument("--device", default="auto")
    args = ap.parse_args()
    t0 = time.time()
    from catspace.research.tools.training_infra.train.scaffold import resolve_device
    dev = resolve_device(args.device)
    from catspace.approaches.gauntlet_harness.experiments.play_vs_maia import CommittorGreedy
    from catspace.research.components.planner.approaches.subgoal_cascade.experiments.m4_play_steering import maia_feats
